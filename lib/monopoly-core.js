...
function canUpgrade(room, player, index) {
  ...
  const state = propertyState(room, index)
  if (!tileInfo || tileInfo.type !== "property") return { ok: false, message: "Petak ini tidak dapat di-upgrade." }
  if (String(state.owner) !== String(player.jid)) return { ok: false, message: "Properti ini bukan milikmu." }
  if (state.level >= MAX_PROPERTY_LEVEL) return { ok: false, message: "Properti ini sudah menjadi hotel." }
  if (!ownsFullGroup(room, player, tileInfo.group)) return { ok: false, message: "Kamu harus memiliki semua properti satu warna untuk upgrade." }
  const levels = groupTiles(tileInfo.group).map((entry) => propertyState(room, entry.index).level)
  const minLevel = Math.min(...levels)
  if (state.level > minLevel) return { ok: false, message: "Upgrade harus seimbang dalam satu kelompok warna." }
  return { ok: true }
}
...
function resolveLanding(room, actorIndex, diceTotal, context = {}, depth = 0) {
  ...
  if (["property", "station", "utility"].includes(tileInfo.type)) {
    ...
    const paid = debit(room, player, rent, owner)
    landing.rent = paid
    landing.owner = owner ? playerSummary(owner) : null
    landing.messages.push("Membayar sewa M" + paid)
    return landing
  }
...
export function rollRoom(room, sender) {
  ...
  if (actor.inJail) {
    if (actor.jailFreeCards?.length) {
      const saved = actor.jailFreeCards.shift()
      returnJailFreeCard(room, saved)
      actor.inJail = false
      actor.jailAttempts = 0
      const movement = moveBy(room, actor, total)
      const landing = resolveLanding(room, actorIndex, total)
      if (!room.pendingAction) finishTurn(room, doubles && !actor.inJail)
      updateWinner(room)
      const event = { type: "roll", actor: playerSummary(actor), dice, from: { ...tileAt(movement.before) }, to: { ...tileAt(actor.position) }, passedStart: movement.passedStart, landing, extraTurn: doubles && !room.pendingAction, usedJailCard: true, next: playerSummary(currentPlayer(room)), message: actor.name + " menggunakan kartu Bebas dari Penjara." }
      room.lastEvent = event
      return { ok: true, event }
    }
    ...
    if (doubles) {
      ...
      const movement = moveBy(room, actor, total)
      const landing = resolveLanding(room, actorIndex, total)
      if (!room.pendingAction) finishTurn(room, true)
      updateWinner(room)
      const event = { type: "roll", actor: playerSummary(actor), dice, from: { ...tileAt(movement.before) }, to: { ...tileAt(actor.position) }, passedStart: movement.passedStart, landing, extraTurn: !room.pendingAction, releasedFromJail: true, next: playerSummary(currentPlayer(room)), message: actor.name + " keluar dari Penjara dengan dadu kembar." }
      room.lastEvent = event
      return { ok: true, event }
    }

    actor.jailAttempts += 1
    if (actor.jailAttempts >= 3) {
      debit(room, actor, JAIL_FINE)
      actor.inJail = false
      actor.jailAttempts = 0
      const movement = moveBy(room, actor, total)
      const landing = resolveLanding(room, actorIndex, total)
      ...
      updateWinner(room)
      const event = { type: "roll", actor: playerSummary(actor), dice, from: { ...tileAt(movement.before) }, to: { ...tileAt(actor.position) }, passedStart: movement.passedStart, landing, paidJailFine: true, next: playerSummary(currentPlayer(room)), message: actor.name + " membayar denda Penjara M" + JAIL_FINE + "." }
      room.lastEvent = event
      return { ok: true, event }
    }
    ...
    const event = { type: "jail_wait", actor: playerSummary(actor), dice, jailAttempts: actor.jailAttempts, next: playerSummary(currentPlayer(room)), message: actor.name + " belum berhasil keluar dari Penjara." }
    room.lastEvent = event
    return { ok: true, event }
  }

  actor.doublesStreak = doubles ? actor.doublesStreak + 1 : 0
  if (actor.doublesStreak >= 3) {
    goToJail(room, actor)
    finishTurn(room, false)
    const event = { type: "jail", actor: playerSummary(actor), dice, next: playerSummary(currentPlayer(room)), message: actor.name + " mendapat tiga dadu kembar dan masuk Penjara." }
    room.lastEvent = event
    return { ok: true, event }
  }
...
export function buyRoom(room, sender) {
  ...
  if (!pending || pending.type !== "buy") return { ok: false, message: "Tidak ada properti yang menunggu untuk dibeli." }
  ...
  const tileInfo = MONOPOLY_BOARD[index]
  const state = propertyState(room, index)
  if (!actor || !tileInfo || !["property", "station", "utility"].includes(tileInfo.type)) return { ok: false, message: "Data properti tidak valid." }
  ...
  finishTurn(room, false)
  updateWinner(room)
  const event = { type: "buy", actor: playerSummary(actor), tile: { ...tileInfo }, level: state.level, next: playerSummary(currentPlayer(room)), message: actor.name + " membeli " + tileInfo.name + "." }
  room.lastEvent = event
  return { ok: true, event }
}
...
export function passRoom(room, sender) {
  ...
  room.pendingAction = null
  finishTurn(room, false)
  const event = { type: "pass", actor: playerSummary(actor), tile: tileInfo ? { ...tileInfo } : null, skippedType, next: playerSummary(currentPlayer(room)), message: actor.name + " melewati pilihan di " + (tileInfo?.name || "petak") + "." }
  room.lastEvent = event
  return { ok: true, event }
}
